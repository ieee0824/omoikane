...
- **新機能には必ずテストを追加する** — テストなしのコミットは禁止
- **レビュー指摘で「テスト不足」と言われたら必ず追加する** — 「軽微」を理由にスキップしない
- **テストは具体的なアサーションを書く** — `assert!(count > 0)` のような弱いアサーションではなく、期待値を明示する
- **既存テストを削除・弱化しない** — テストを通すために閾値を緩めたりアサーションを削除しない
