...
pub fn parse_fragment(bytes: &[u8], context: &NodeHandle) -> Result<NodeHandle, XmlParseError> {
    ...
    for (prefix, namespace) in declarations {
        wrapper.push_str(if prefix.is_empty() {
            " xmlns=\""
        } else {
            " xmlns:"
        });
...
impl<'a> Parser<'a> {
    ...
    fn parse_document(mut self) -> Result<NodeHandle, XmlParseError> {
        ...
        if self.starts("<?xml") {
                ...
                && !enc.eq_ignore_ascii_case("utf8")
            {
                return Err(XmlParseError::new(
                    "XML declaration encoding conflicts with UTF-8 input",
                ));
            }
        }
        ...
        while self.pos < self.input.len() {
            if self.starts("<!--") {
                ...
                if self.stack.is_empty() {
                    if root_seen {
                        return self.err("multiple document elements");
                    }
                    root_seen = true;
                }
                self.parse_start()?;
        ...
        if !root_seen {
            return self.err("missing document element");
        }
        Ok(self.document)
    }
    ...
    fn parse_start(&mut self) -> Result<(), XmlParseError> {
        ...
        loop {
            ...
            if attributes
                .iter()
                .any(|(n, _): &(String, String)| n == &attr_name)
            {
                return self.err("duplicate attribute");
            }
            self.ws();
            if !self.consume("=") {
                return self.err("attribute value must follow '='");
            }
            self.ws();
            let quote = self
                .peek()
                .ok_or_else(|| XmlParseError::new("missing attribute value"))?;
            if quote != '\'' && quote != '"' {
                return self.err("XML attribute values must be quoted");
            }
            ...
            if attr_name == "xmlns" {
                ...
                if prefix.is_empty() {
                    return self.err("empty namespace prefix");
                }
                if prefix == "xmlns" {
                    return self.err("the xmlns prefix is reserved");
                }
        ...
        for (attr, value) in attributes {
            let (attr_prefix, local_name) = attr
                .split_once(':')
                .map(|(prefix, local)| (Some(prefix), local))
                .unwrap_or((None, attr.as_str()));
            let attribute_namespace =
                if attr == "xmlns" || attr_prefix == Some("xmlns") {
                    Some(XMLNS_NS.to_string())
                } else if let Some(prefix) = attr_prefix {
                    Some(namespaces.get(prefix).cloned().ok_or_else(|| {
                        XmlParseError::new("undeclared attribute namespace prefix")
                    })?)
                } else {
                    None
                };
    ...
    fn parse_end(&mut self) -> Result<(), XmlParseError> {
        ...
        let open = self
            .stack
            .pop()
            .ok_or_else(|| XmlParseError::new("unexpected end tag"))?;
        if open.name != name {
            return self.err("mismatched end tag");
        }
        Ok(())
    }
    ...
    fn parse_text(&mut self) -> Result<(), XmlParseError> {
        ...
        if self.stack.is_empty() {
            if !text.trim().is_empty() {
                return self.err("text outside document element");
            }
    ...
    fn parse_comment(&mut self) -> Result<(), XmlParseError> {
        ...
        self.parent().append_child(NodeHandle::comment(body));
        Ok(())
    }
    ...
    fn parse_cdata(&mut self) -> Result<(), XmlParseError> {
        if self.stack.is_empty() {
            return self.err("CDATA outside document element");
        }
        self.pos += 9;
    ...
    fn parse_doctype(&mut self, root_seen: bool) -> Result<(), XmlParseError> {
        if root_seen || !self.stack.is_empty() {
            return self.err("misplaced doctype");
        }
        self.pos += 9;
        self.ws();
        let name = self.name()?;
        ...
        if self.consume("PUBLIC") {
            self.ws();
            public_id = self.quoted_literal()?;
            self.ws();
            system_id = self.quoted_literal()?;
            self.ws();
        } else if self.consume("SYSTEM") {
            self.ws();
            system_id = self.quoted_literal()?;
            self.ws();
        }
        if self.peek() == Some('[') {
            return self.err("internal subsets are unsupported");
        }
        if !self.consume(">") {
            return self.err("malformed doctype");
        }
        self.document
            .append_child(NodeHandle::document_type(name, public_id, system_id));
        Ok(())
    }
    ...
    fn quoted_literal(&mut self) -> Result<String, XmlParseError> {
        let q = self
            .peek()
            .ok_or_else(|| XmlParseError::new("missing quoted literal"))?;
        if q != '\'' && q != '"' {
            return self.err("doctype identifier must be quoted");
        }
        self.pos += 1;
        Ok(self.take_char_until(q)?.to_string())
    }
    fn parent(&self) -> NodeHandle {
        self.stack
            .last()
            .map(|e| e.node.clone())
    ...
    fn name(&mut self) -> Result<String, XmlParseError> {
        ...
        while let Some(ch) = self.peek() {
            if is_name_char(ch, self.pos == start) {
        ...
        if name.matches(':').count() > 1 || name.starts_with(':') || name.ends_with(':') {
            return self.err("malformed qualified name");
        }
        Ok(name.to_string())
    }
    fn ws(&mut self) {
    ...
    fn take_until(&mut self, end: &str) -> Result<&'a str, XmlParseError> {
        ...
        let offset = self.input[start..]
            .find(end)
            .ok_or_else(|| XmlParseError::new("unterminated XML construct"))?;
        self.pos = start + offset + end.len();
        Ok(&self.input[start..start + offset])
    }
    fn take_char_until(&mut self, end: char) -> Result<&'a str, XmlParseError> {
        let start = self.pos;
        let offset = self.input[start..]
            .find(end)
            .ok_or_else(|| XmlParseError::new("unterminated quoted value"))?;
        self.pos = start + offset + end.len_utf8();
        Ok(&self.input[start..start + offset])
    }
    fn err<T>(&self, msg: &str) -> Result<T, XmlParseError> {
        Err(XmlParseError::new(format!("{msg} at byte {}", self.pos)))
    }
}

fn declaration_encoding(decl: &str) -> Option<&str> {
    let pos = decl.find("encoding")?;
    let rest = decl[pos + 8..].trim_start();
...
fn is_name_char(ch: char, first: bool) -> bool {
    ch == ':'
        || ch == '_'
        || ch.is_alphabetic()
        || (!first && (ch.is_ascii_digit() || matches!(ch, '-' | '.') || ch == '\u{b7}'))
}
...
fn decode_references(input: &str) -> Result<String, XmlParseError> {
    ...
    while let Some(pos) = rest.find('&') {
        ...
        let end = rest
            .find(';')
            .ok_or_else(|| XmlParseError::new("unterminated entity reference"))?;
        let entity = &rest[..end];
        rest = &rest[end + 1..];
        let value = match entity {
            ...
            value if value.starts_with("#x") => char::from_u32(
                u32::from_str_radix(&value[2..], 16)
                    .map_err(|_| XmlParseError::new("invalid numeric character reference"))?,
            )
            .ok_or_else(|| XmlParseError::new("invalid XML character"))?,
...
mod tests {
    ...
    fn root(doc: &NodeHandle) -> NodeHandle {
        doc.child_nodes()
            .into_iter()
    ...
    #[test]
    fn parses_namespaces_entities_cdata_and_preserves_case() {
        ...
        assert_eq!(root.namespace_uri().as_deref(), Some("urn:d"));
        assert_eq!(
            root.attributes().unwrap().get("A").map(String::as_str),
            Some("<&AB")
        );
        let child = root
            .child_nodes()
            ...
            .find(|n| n.node_type() == NodeType::Element)
            .unwrap();
        assert_eq!(child.tag_name().as_deref(), Some("p:Child"));
        assert_eq!(child.namespace_uri().as_deref(), Some("urn:p"));
        assert_eq!(child.local_name().as_deref(), Some("Child"));
    }
    #[test]
    fn resolves_attribute_namespaces_and_local_names() {
        let doc =
            parse(br#"<outer xmlns:a='urn:attribute'><inner a:Mixed='value'/></outer>"#).unwrap();
        let inner = root(&doc)
            .child_nodes()
            .into_iter()
            ...
            .unwrap();
        assert_eq!(
            inner.attribute_records().unwrap(),
            vec![(
                "a:Mixed".into(),
    ...
    #[test]
    fn accepts_doctype_comment_pi_and_defined_entities() {
        assert!(
            parse(br#"<?x ok?><!DOCTYPE R SYSTEM 'urn:x'><!--c--><R>&gt;&quot;&apos;</R>"#).is_ok()
        );
    }
    #[test]
    fn rejects_processing_instruction_data_without_whitespace() {
        assert!(parse(br#"<r><?x,data?></r>"#).is_err());
        assert!(parse(br#"<r><?x ok?></r>"#).is_ok());
        assert!(parse(br#"<r><?x?></r>"#).is_ok());
    }
    #[test]
    fn rejects_mismatched_tags_unquoted_attributes_and_unknown_entities() {
        for xml in ["<a></b>", "<a x=y/>", "<a>&bogus;</a>"] {
            assert!(parse(xml.as_bytes()).is_err(), "expected error for {xml}");
        }
    }
    #[test]
    ...
    #[test]
    fn rejects_invalid_utf8_and_non_utf8_declaration() {
        assert!(parse(b"<r>\xff</r>").is_err());
        assert!(parse(br#"<?xml version='1.0' encoding='ISO-8859-1'?><r/>"#).is_err());
    }
    #[test]
    fn rejects_xhtml_crossed_tags_as_whole_document_error() {
        assert!(parse(br#"<html><p><strong/>x</strong></p></html>"#).is_err());
    }
    #[test]
    fn rejects_invalid_reserved_namespace_prefix_bindings() {
        for xml in [
            ...
            assert!(
                ...
                "expected fatal error for {xml}"
            );
        }
        assert!(
            parse(format!("<r xmlns:xml='{XML_NS}'/>").as_bytes()).is_ok(),
            "the predefined xml prefix may bind to the XML namespace"
        );
    }
    #[test]
    fn rejects_reserved_xml_processing_instruction_target_case_insensitively() {
        ...
        assert!(
            parse(b"<r><?xml-stylesheet ok?></r>").is_ok(),
            "a target merely starting with xml remains valid"
        );
    }
    #[test]
    fn rejects_double_hyphen_and_trailing_hyphen_in_comments() {
        ...
        assert!(
            parse(b"<r><!--a-b--></r>").is_ok(),
            "a single interior hyphen is valid"
        );
    }
    #[test]
    fn rejects_xml_1_0_forbidden_characters_directly_and_by_reference() {
        for xml in [
            "<r>\u{1}</r>",
            "<r>\u{b}</r>",
            "<r>\u{fffe}</r>",
            ...
            assert!(
                parse(xml.as_bytes()).is_err(),
                "expected invalid XML character error for {xml:?}"
            );
        }
        assert!(
