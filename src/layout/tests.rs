...
/// Finds the layout box generated by `node`, if it produced one.
fn find_layout_box<'a>(layout: &'a LayoutBox, node: &NodeHandle) -> Option<&'a LayoutBox> {
    if &layout.node == node {
        return Some(layout);
    }
    layout
...
fn multicol_text_fixture(style: &str, line_count: usize) -> (LayoutBox, NodeHandle) {
    ...
    for index in 0..line_count {
        container.append_child(NodeHandle::text(format!("line{index}")));
        if index + 1 < line_count {
            container.append_child(NodeHandle::element("br"));
        }
    }
...
#[test]
fn multicol_balances_direct_lines_across_used_columns() {
    let (layout, container) = multicol_text_fixture(
        "width:220px;columns:2;column-gap:20px;font-size:10px;line-height:20px",
        4,
    );
    let multicol = find_layout_box(&layout, &container).unwrap();
    assert_eq!(multicol.lines.len(), 4);
    assert_eq!(multicol.dimensions.content.height, 40.0);
    let positions = multicol
        .lines
        .iter()
        .map(|line| (line.rect.x, line.rect.y))
        .collect::<Vec<_>>();
    assert_eq!(
        positions,
        vec![(0.0, 0.0), (0.0, 20.0), (120.0, 0.0), (120.0, 20.0)]
    );
}

#[test]
fn multicol_auto_fill_uses_explicit_height_and_creates_overflow_columns() {
    let (layout, container) = multicol_text_fixture(
        "width:220px;height:40px;columns:2;column-gap:20px;column-fill:auto;font-size:10px;line-height:20px",
        6,
    );
    let multicol = find_layout_box(&layout, &container).unwrap();
    assert_eq!(multicol.dimensions.content.height, 40.0);
    assert_eq!(multicol.lines[0].rect.x, 0.0);
    assert_eq!(multicol.lines[2].rect.x, 120.0);
    assert_eq!(multicol.lines[4].rect.x, 240.0);
    assert!(multicol.scrollable_overflow().0 >= 340.0);
}
...
fn multicol_balance_grows_to_keep_indivisible_blocks_in_the_requested_columns() {
    ...
    let positions = children
        .iter()
        .map(|child| {
            let layout = find_layout_box(&layout, child).unwrap();
            (layout.dimensions.content.x, layout.dimensions.content.y)
        })
        .collect::<Vec<_>>();
...
#[test]
fn multicol_fragments_block_lines_and_honors_orphans_widows_and_avoid() {
    let layout_case = |paragraph_style: &str, line_count: usize| {
        let document = NodeHandle::document();
        let body = NodeHandle::element("body");
        let container = NodeHandle::element("main");
        ...
        document.append_child(body.clone());
        body.append_child(container.clone());
        container.append_child(paragraph.clone());
        for index in 0..line_count {
            paragraph.append_child(NodeHandle::text(format!("line{index}")));
            if index + 1 < line_count {
                paragraph.append_child(NodeHandle::element("br"));
            }
        }
        container.set_attribute(
            "style",
            "width:340px;height:40px;columns:3;column-gap:20px;column-fill:auto",
        );
        paragraph.set_attribute(
            "style",
        ...
        .unwrap();
        let paragraph = find_layout_box(&layout, &paragraph).unwrap();
        paragraph
            .lines
            .iter()
            .map(|line| (line.rect.x, line.rect.y))
    ...
    assert_eq!(
        layout_case("orphans:2;widows:2", 5),
        vec![
            (0.0, 0.0),
            (0.0, 20.0),
            (120.0, 0.0),
    ...
    assert_eq!(
        layout_case("break-inside:avoid-column", 5),
        vec![
            (0.0, 0.0),
            (0.0, 20.0),
            (0.0, 40.0),
...
#[test]
fn multicol_honors_forced_column_breaks_and_full_width_spanners() {
    let document = NodeHandle::document();
    let body = NodeHandle::element("body");
    let container = NodeHandle::element("main");
    let first = NodeHandle::element("section");
    let second = NodeHandle::element("section");
    let spanner = NodeHandle::element("h2");
    let after = NodeHandle::element("article");
    document.append_child(body.clone());
    body.append_child(container.clone());
    container.append_child(first.clone());
    container.append_child(second.clone());
    container.append_child(spanner.clone());
    container.append_child(after.clone());
    container.set_attribute(
        "style",
        "width:220px;column-count:2;column-gap:20px;column-fill:balance",
    );
    ...
    assert!(after_box.dimensions.content.y >= spanner_box.dimensions.content.y + 10.0);
}

#[test]
fn multicol_measures_spanner_height_at_the_full_container_width() {
    let document = NodeHandle::document();
    let body = NodeHandle::element("body");
    let container = NodeHandle::element("main");
    let spanner = NodeHandle::element("h2");
    ...
    container.append_child(spanner.clone());
    container.append_child(after.clone());
    spanner.append_child(NodeHandle::text("abcdefghijklmnopqrstuvwxyz"));
    container.set_attribute("style", "width:220px;columns:2;column-gap:20px");
    spanner.set_attribute(
        "style",
    ...
    let after_box = find_layout_box(&layout, &after).unwrap();
    assert_eq!(spanner_box.dimensions.content.width, 220.0);
    assert_eq!(spanner_box.dimensions.content.height, 20.0);
    assert_eq!(after_box.dimensions.content.y, 20.0);
}

#[test]
fn vertical_multicol_balances_lines_along_the_physical_inline_axis() {
    let (layout, container) = multicol_text_fixture(
        "writing-mode:vertical-rl;width:40px;height:220px;columns:2;column-gap:20px;\
         column-rule:4px solid red;font-size:10px;line-height:20px",
        4,
    );
    let multicol = find_layout_box(&layout, &container).unwrap();
    assert_eq!(multicol.lines.len(), 4);
    ...
    assert_eq!(
        positions,
        vec![(20.0, 0.0), (0.0, 0.0), (20.0, 120.0), (0.0, 120.0)]
    );
    let metadata = multicol.multicol.as_ref().expect("multicol metadata");
    assert_eq!(metadata.rules.len(), 1);
    assert_eq!(metadata.rules[0].y, 110.0);
    assert_eq!(metadata.rules[0].width, 40.0);
}
...
#[test]
fn absolute_image_does_not_generate_an_inline_fragment_or_line_box() {
    let document = NodeHandle::document();
    let body = NodeHandle::element("body");
    ...
    resolver.add_stylesheet(
        Origin::Author,
        parse_stylesheet(
            "body { margin: 0; line-height: 80px; } \
             img { position: absolute; left: 0; top: 0; width: 2px; height: 1px; }",
        )
        .unwrap(),
    );
    let layout = layout_tree(
    ...
    assert!(image_box.lines.is_empty());
    assert_eq!(image_box.dimensions.content.height, 1.0);
}

#[test]
fn static_inline_image_still_generates_an_image_fragment() {
    let document = NodeHandle::document();
    let body = NodeHandle::element("body");
    let image = NodeHandle::element("img");
    image.set_attribute(
...
#[test]
fn size_containment_uses_zero_intrinsic_and_auto_block_size() {
    let document = NodeHandle::document();
    let body = NodeHandle::element("body");
    let contained = NodeHandle::element("section");
    let child = NodeHandle::element("div");
    contained.set_attribute("class", "contained");
    document.append_child(body.clone());
    body.append_child(contained.clone());
    contained.append_child(child);

    let mut resolver = StyleResolver::new();
    resolver.add_stylesheet(
        Origin::Author,
        parse_stylesheet(
            "body { margin: 0; } .contained { float: left; contain: size; } \
             .contained > div { width: 80px; height: 40px; }",
        )
        .unwrap(),
    );
    let layout = layout_tree(
    ...
    assert_eq!(box_.dimensions.content.width, 0.0);
    assert_eq!(box_.dimensions.content.height, 0.0);
    assert_eq!(box_.children[0].dimensions.content.height, 40.0);
}
...
fn layout_containment_establishes_positioned_descendant_boundary() {
    ...
    let child = NodeHandle::element("div");
    contained.set_attribute("class", "contained");
    child.set_attribute("class", "positioned");
    document.append_child(body.clone());
    body.append_child(contained.clone());
    contained.append_child(child);
    ...
    resolver.add_stylesheet(
        Origin::Author,
        parse_stylesheet(
            "body { margin: 0; } .contained { margin-left: 30px; width: 100px; height: 50px; contain: layout; } \
             .positioned { position: absolute; right: 0; width: 10px; height: 10px; }",
        )
        .unwrap(),
    );
    let layout = layout_tree(
    ...
    assert_eq!(positioned.dimensions.content.x, 120.0);
}
...
fn scrollable_overflow_keywords_establish_a_scroll_container() {
    ...
    for (value, expected) in [
        ...
        resolver.add_stylesheet(
            Origin::Author,
            parse_stylesheet(&format!(
                "div {{ display: block; width: 50px; height: 20px; overflow: {value}; }}"
            ))
            .unwrap(),
        );
        let _ = body;
...
fn scroll_offset_does_not_change_layout_geometry() {
    ...
    resolver.add_stylesheet(
        ...
        parse_stylesheet(
            "body { margin: 0; } \
             div { display: block; width: 50px; height: 20px; overflow: hidden; } \
...
fn scroll_offset_is_clamped_to_the_scrollable_extent() {
    ...
    resolver.add_stylesheet(
        ...
        parse_stylesheet(
            "body { margin: 0; } \
             div { display: block; width: 100px; height: 50px; overflow: hidden; } \
             p { display: block; width: 300px; height: 200px; margin: 0; }",
        )
        .unwrap(),
    );
    ...
    // scrollWidth 300 / scrollHeight 200 against a 100x50 padding box.
    assert_eq!(card_box.scrollable_overflow(), (300.0, 200.0));
    assert_eq!(card_box.max_scroll_offset(), (200.0, 150.0));

    card.set_scroll_offset(20.0, 30.0);
    assert_eq!(card_box.scroll_offset(), (20.0, 30.0));

    // Stored offsets beyond the extent report the extent, without rewriting the
    // stored value.
    card.set_scroll_offset(9999.0, 9999.0);
    assert_eq!(card_box.scroll_offset(), (200.0, 150.0));
    assert_eq!(card.scroll_offset(), (9999.0, 9999.0));

    // Negative offsets and non-scroll containers report nothing.
    card.set_scroll_offset(-5.0, -5.0);
    assert_eq!(card_box.scroll_offset(), (0.0, 0.0));
    let inner_box = find_layout_box(&layout, &inner).unwrap();
    inner.set_scroll_offset(10.0, 10.0);
    assert_eq!(inner_box.scroll_offset(), (0.0, 0.0));
}

#[test]
fn scrollable_overflow_stops_at_nested_scroll_containers() {
    let (document, _html, body, card) = sample_tree();
    let nested = NodeHandle::element("section");
    let deep = NodeHandle::element("p");
    card.append_child(nested.clone());
    nested.append_child(deep.clone());
    let mut resolver = StyleResolver::new();
    resolver.add_stylesheet(
        Origin::Author,
        parse_stylesheet(
            "body { margin: 0; } \
             div { display: block; width: 100px; height: 100px; overflow: hidden; } \
             section { display: block; width: 60px; height: 60px; overflow: auto; } \
             p { display: block; width: 500px; height: 500px; margin: 0; }",
        )
        .unwrap(),
    );
    ...
    // The nested container bounds its own subtree, so the outer container only
    // sees the nested border box, not the 500x500 grandchild.
    let card_box = find_layout_box(&layout, &card).unwrap();
    assert_eq!(card_box.scrollable_overflow(), (100.0, 100.0));
    assert_eq!(card_box.max_scroll_offset(), (0.0, 0.0));
    let nested_box = find_layout_box(&layout, &nested).unwrap();
