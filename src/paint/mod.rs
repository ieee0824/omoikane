...
fn paint_cloned_fragment_decorations(
    ...
    let decoration = LayoutBox {
        ...
        position_scheme: layout.position_scheme,
        fixed_containing_block: layout.fixed_containing_block,
        z_index: layout.z_index,
        transform: layout.transform,
        needs_scroll_translation: false,
        has_out_of_flow_descendants: layout.has_out_of_flow_descendants,
...
fn background_repeat(style: &ComputedStyle) -> (bool, bool) {
    let value = match style.get("background-repeat") {
    ...
    if let PreparedBackgroundImage::Gradient(gradient) = background {
        ...
        if has_explicit_size {
            ...
            let tile_image = if repeat_x || repeat_y {
                ...
                if u64::from(tw) * u64::from(th) <= MAX_TILE_PIXELS {
