...
- コミットは変更目的ごとにまとめる。PRには問題、変更後の振る舞い、検証結果、未解決事項を書く。
- PRの更新時は、タイトルと本文を最終的な変更内容に合わせる。
- **GitHub Copilotのレビューは不要。レビューを依頼せず、レビュー完了や承認を待つことをPR作成・マージの条件にしない。**
- 自分で差分を確認し、必要なCI結果と関連する不具合を確認する。
- マージはユーザーから依頼されている場合に行う。レビュー不要という方針は、CIやテストの失敗を無視する意味ではない。
