...
- HashMap rule index適用後: median 2.506ms（候補フィルタから86.8%、初期から97.5%短縮）

top-level style ruleはID・class・tag・fallbackのHashMap indexから直接列挙する。
`@media`など条件付き規則は既存の再帰評価を維持し、事前計算した宣言offsetでsource orderを保つ。
