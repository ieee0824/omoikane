...
impl IntervalInnerState {
    ...
    /// present.
    fn from_context(context: &mut Context) -> GcEdge<GcRefCell<Self>> {
        if !context.has_data::<GcEdge<GcRefCell<IntervalInnerState>>>() {
            context.insert_data(GcEdge::new(GcRefCell::new(Self::default())));
        }

        context
            .get_data::<GcEdge<GcRefCell<Self>>>()
...
fn handle(
    handler_map: GcEdge<GcRefCell<IntervalInnerState>>,
    id: u32,
